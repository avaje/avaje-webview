...
/**
 * Win32 API + WebView2 Panama downcall handles.
 */
final class Win32 {
  ...
  // MSG layout: HWND, UINT, WPARAM, LPARAM, DWORD, POINT{x,y}
  ...
  static final MethodHandle GetWindowLongPtrW   = downcall(USER32,  "GetWindowLongPtrW",   FunctionDescriptor.of(JAVA_LONG, ADDRESS, JAVA_INT));
  static final MethodHandle SetWindowLongPtrW   = downcall(USER32,  "SetWindowLongPtrW",   FunctionDescriptor.of(JAVA_LONG, ADDRESS, JAVA_INT, JAVA_LONG));
  static final MethodHandle CallWindowProcW     = downcall(USER32,  "CallWindowProcW",     FunctionDescriptor.of(JAVA_LONG, ADDRESS, ADDRESS, JAVA_INT, JAVA_LONG, JAVA_LONG));
  ...
  static final MethodHandle DwmSetWindowAttribute = downcall(DWMAPI,"DwmSetWindowAttribute",FunctionDescriptor.of(JAVA_INT, ADDRESS, JAVA_INT, ADDRESS, JAVA_INT));

  private static MethodHandle downcall(SymbolLookup lib, String sym, FunctionDescriptor desc) {
    return LINKER.downcallHandle(
        lib.find(sym).orElseThrow(() -> new UnsatisfiedLinkError("Win32 symbol not found: " + sym)),
        desc);
  }
  ...
  static void postQuitMessage(int exitCode) {
    try { PostQuitMessage.invokeExact(exitCode); }
    catch (Throwable t) { throw new RuntimeException(t); }
  }
  ...
  static void fullscreen(MemorySegment hwnd) {
    try (Arena a = Arena.ofConfined()) {
      ...
      int _ = (int) GetWindowRect.invokeExact(hwnd, rect);
      ...
      int _ = (int) SetWindowPos.invokeExact(hwnd, MemorySegment.NULL,
          0, 0, screenW, screenH, SWP_NOZORDER | SWP_SHOWWINDOW);
    } catch (Throwable t) { throw new RuntimeException(t); }
  }

  static void setDarkMode(MemorySegment hwnd, boolean dark) {
    try (Arena a = Arena.ofConfined()) {
      MemorySegment boolVal = a.allocate(JAVA_INT);
      boolVal.set(JAVA_INT, 0, dark ? 1 : 0);
      int _ = (int) DwmSetWindowAttribute.invokeExact(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE,
          boolVal, (int) JAVA_INT.byteSize());
  ...
  static void setIcon(MemorySegment hwnd, java.nio.file.Path iconPath) {
    try (Arena a = Arena.ofConfined()) {
      String path = iconPath.toAbsolutePath().toString();
      MemorySegment pathSeg = a.allocateFrom(path, java.nio.charset.StandardCharsets.UTF_16LE);
      MemorySegment bigIcon = (MemorySegment) LoadImageW.invokeExact(
          MemorySegment.NULL, pathSeg, IMAGE_ICON, 32, 32, LR_LOADFROMFILE);
      MemorySegment smallIcon = (MemorySegment) LoadImageW.invokeExact(
          MemorySegment.NULL, pathSeg, IMAGE_ICON, 16, 16, LR_LOADFROMFILE);
      if (!bigIcon.equals(MemorySegment.NULL)) {
        long _ = (long) SendMessageW.invokeExact(hwnd, WM_SETICON, (long) ICON_BIG, bigIcon);
      }
      if (!smallIcon.equals(MemorySegment.NULL)) {
        long _ = (long) SendMessageW.invokeExact(hwnd, WM_SETICON, (long) ICON_SMALL, smallIcon);
      }
    } catch (Throwable t) { throw new RuntimeException(t); }
  }
