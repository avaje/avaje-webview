...
final class CocoaWebView extends WebviewBase {
  ...
  static {
    ...
    DISPATCH_MAIN_QUEUE = libDispatch.find("_dispatch_main_q")
        .orElseThrow(() -> new UnsatisfiedLinkError("_dispatch_main_q"));
    DISPATCH_ASYNC_F = linker.downcallHandle(
  ...
  protected void navigateImpl(String url) {
    try (Arena a = Arena.ofConfined()) {
      MemorySegment nsUrl   = send1(getClass(a, "NSURL"), sel(a, "URLWithString:"), nsString(a, url));
      MemorySegment request = send1(getClass(a, "NSURLRequest"), sel(a, "requestWithURL:"), nsUrl);
  ...
  protected void setSizeImpl(int width, int height) {
    try (Arena a = Arena.ofConfined()) {
      MSG_SEND_SET_CONTENT_SIZE.invokeExact(nsWindow, sel(a, "setContentSize:"),
  ...
  protected void setMinSizeImpl(int width, int height) {
    try (Arena a = Arena.ofConfined()) {
      MSG_SEND_SET_SIZE.invokeExact(nsWindow, sel(a, "setMinSize:"),
          (double) width, (double) height);
    } catch (Throwable t) { throw new RuntimeException(t); }
  }
  ...
  protected void setMaxSizeImpl(int width, int height) {
    try (Arena a = Arena.ofConfined()) {
      MSG_SEND_SET_SIZE.invokeExact(nsWindow, sel(a, "setMaxSize:"),
          (double) width, (double) height);
    } catch (Throwable t) { throw new RuntimeException(t); }
  }
  ...
  protected void setHtmlImpl(String html) {
    try (Arena a = Arena.ofConfined()) {
      sendVoid1(wkWebView, sel(a, "loadHTMLString:baseURL:"),
  ...
  protected void evalImpl(String js) {
    try (Arena a = Arena.ofConfined()) {
      MSG_SEND_EVAL_JS.invokeExact(wkWebView, sel(a, "evaluateJavaScript:completionHandler:"),
          nsString(a, js), MemorySegment.NULL);
    } catch (Throwable t) { throw new RuntimeException(t); }
  }
  ...
  protected void nativeAddUserScript(String js) {
    try (Arena a = Arena.ofConfined()) {
      MemorySegment WKUserScript = getClass(a, "WKUserScript");
      MemorySegment script = MSG_SEND_WKUSERSCRIPT_INIT.invokeExact(
          send0(WKUserScript, sel(a, "alloc")),
          sel(a, "initWithSource:injectionTime:forMainFrameOnly:"),
          nsString(a, js),
          WK_INJECT_AT_DOCUMENT_START,
          1 /* mainFrameOnly = YES */);
      sendVoid1(ucController, sel(a, "addUserScript:"), (MemorySegment) script);
    } catch (Throwable t) { throw new RuntimeException(t); }
  }
  ...
  @SuppressWarnings("unused")
  public void onScriptMessage(MemorySegment self, MemorySegment cmd,
  ...
  private void buildDrainStub() {
    try {
      var mh = MethodHandles.lookup().findVirtual(CocoaWebView.class, "drainDispatchQueue",
          MethodType.methodType(void.class, MemorySegment.class)).bindTo(this);
      drainStub = Linker.nativeLinker().upcallStub(mh,
          FunctionDescriptor.ofVoid(ValueLayout.ADDRESS), callbackArena);
  ...
  private static void initNSApp() {
    ...
    try (Arena a = Arena.ofConfined()) {
      ...
      Linker.nativeLinker().downcallHandle(MSG_SEND_ADDR,
  ...
  private void initWindowAndWebView(boolean debug, int width, int height) {
    try (Arena a = Arena.ofConfined()) {
      ...
      MemorySegment WKConfig = getClass(a, "WKWebViewConfiguration");
      ...
      send2(ucController, sel(a, "addScriptMessageHandler:name:"),
          handler, nsString(a, HANDLER_NAME));

      // WKWebView
      wkWebView = MSG_SEND_WKWEBVIEW_INIT.invokeExact(
          send0(getClass(a, "WKWebView"), sel(a, "alloc")),
          sel(a, "initWithFrame:configuration:"),
          0d, 0d, (double) width, (double) height,
          config);
      wkWebView = (MemorySegment) wkWebView;

      // NSWindow
      nsWindow = MSG_SEND_NSWINDOW_INIT.invokeExact(
          send0(getClass(a, "NSWindow"), sel(a, "alloc")),
          sel(a, "initWithContentRect:styleMask:backing:defer:"),
          0d, 0d, (double) width, (double) height,
          NS_STANDARD_WINDOW_MASK, NS_BACKING_BUFFERED, 0 /* defer=NO */);
      nsWindow = (MemorySegment) nsWindow;
  ...
  private MemorySegment createScriptHandler(Arena a) {
    try {
      ...
      MemorySegment superCls = getClass(a, "NSObject");
      // Use a unique class name per instance to avoid conflicts
      String clsName = "JavaWebviewHandler_" + System.identityHashCode(this);
      MemorySegment cls = (MemorySegment) ALLOC_CLASS_PAIR.invokeExact(superCls,
          a.allocateFrom(clsName), 0L);

      // Build upcall stub for the handler method
      var mh = MethodHandles.lookup().findVirtual(CocoaWebView.class, "onScriptMessage",
          MethodType.methodType(void.class,
              MemorySegment.class, MemorySegment.class, MemorySegment.class, MemorySegment.class))
          .bindTo(this);
      MemorySegment stub = Linker.nativeLinker().upcallStub(mh,
      ...
      // Type encoding: "v@:@@"
      byte _ = (byte) CLASS_ADD_METHOD.invokeExact(cls,
          sel(a, "userContentController:didReceiveScriptMessage:"),
          stub, a.allocateFrom("v@:@@"));
