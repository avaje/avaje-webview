 ...
 *
 * Every ObjC method call compiles to objc_msgSend(receiver, selector, args...).
 * Panama's invokeExact needs exact types, so we need a separate MethodHandle for each
 * distinct argument signature (can't use varargs). The MSG_SEND_N handles cover all-pointer
 * calls; the named ones handle calls with struct args (NSRect/NSSize as inline doubles) or
 * primitive types (NSInteger, BOOL).
 *
 * ADDRESS = pointer (id, SEL, Class, etc.) = MemorySegment on the Java side.
 * Arena scopes off-heap memory lifetime — ofConfined() for short-lived call args,
 * global() for things that need to live forever.
 */
final class ObjC {
   ...
   * Returns an autoreleased NSString — valid for the current run-loop drain, which is
   * long enough for passing as a method argument in the same arena scope.
   */
  ...
  /**
   * -[NSString UTF8String] → Java String.
   * reinterpret(MAX_VALUE) gives Panama permission to read past the declared segment bounds
   * (the C string was allocated by native code, so Panama has no size metadata for it).
   */
